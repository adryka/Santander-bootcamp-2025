...
@router.get(path="/{id}", status_code=status.HTTP_200_0K)
...
@router.get(path="/", status_code=status.HTTP_200_0K)
...
async def patch(id: UUID4 = Path(alias="id"), body: ProductUpdate = Body(...), usecase: ProductUsecase = Depends ()) -> ProductOut:
    ...
    except NotFoundException as exc: # Captura a exceção NotFoundException
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=exc.message)
...
async def post(body: ProductIn = Body(...), usecase: ProductUsecase = Depends()) -> ProductOut:
    try:
        return await usecase.create(body=body)
    except Exception as exc: # Captura qualquer exceção genérica durante a criação
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error creating product: {exc}"
        )