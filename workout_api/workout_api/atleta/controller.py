...
from workout_api.atleta.schemas import AtletaIn, AtletaOut, AtletaUpdate, AtletaGetAllOut # MUDOU
...
async def post(
    ...
    except IntegrityError: # MUDOU
        raise HTTPException(
            # Usando 303 conforme solicitado.
...
@router.get(
    ...
    # MUDOU - Usando o novo schema de resposta e o tipo de página de paginação
    response_model=LimitOffsetPage[AtletaGetAllOut],
)
async def query(
    db_session: DatabaseDependency,
    nome: Optional[str] = None, # NOVO - Query Parameter
    cpf: Optional[str] = None    # NOVO - Query Parameter
) -> LimitOffsetPage[AtletaGetAllOut]:
    ...
    # Mapeia os resultados para o schema de saída customizado
    atletas_out = [
        AtletaGetAllOut(
            nome=atleta.nome,
    ...
    # Usa a função paginate para aplicar limit/offset
